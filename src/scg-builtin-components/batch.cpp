  ...
  std::shared_ptr<OperatorNode> OperatorBatchNode::make_operator_batch(const YAML::Node& _batch_node, const OperatorNodeFlavor& flavor)
  {
    ...
    else
    {
      ...
      if( batch_node["unroll"] && batch_node["loop"] && batch_node["body"] )
      {
        ...
        if( loop && ul>1 )
        {
          ::onika::ldbg << "unroll "<<tmp_operator_name<<" x"<<ul<<std::endl;
      ...
      if( batch_node["condition"] )
      {
        ...
        if( s1 == "not" )
        {
          ...
          if( OperatorNodeFactory::debug_verbose_level() >= 2 ) { ::onika::ldbg << "condition: ! "<< s2 << std::endl; }
        }
        else
        {
          batch->set_condition(s1);
          batch->set_condition_inverse(false);
          if( OperatorNodeFactory::debug_verbose_level() >= 2 ) { ::onika::ldbg << "condition: "<< s1 << std::endl; }
        }
      }
      ...
      {
        ::onika::ldbg << "concurent parallel batch '"<<tmp_operator_name<<"'"<< std::endl;
      ...
      if( batch_node["task_group"] )
      {
        ...
        ::onika::ldbg << "task group mode for batch '"<<tmp_operator_name<<"' = "<<std::boolalpha<<tg <<std::endl;
      }
...
//    lout<<"========== building "<< tmp_operator_name <<" (" << batch->name() <<") ==========\n";
    ...
    for(YAML::Node op : oplist_node)
    {
      ...
      {
        ::onika::ldbg << "batch "<<tmp_operator_name<<" add operator "<<opname<<" with flavor :" << std::endl;
        for(const auto& p : f)
        {
          if( p.first.find("__") != 0 )
          {
            ::onika::ldbg << '\t' << p.first << " -> " << onika::pretty_short_type(p.second) << std::endl;
          }
        }
      }
