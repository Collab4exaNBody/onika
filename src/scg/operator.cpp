...
#   endif
  ...
  void OperatorNode::run_prolog()
  {
    ...
    if( do_profiling )
    {
      if( m_gpu_profile_start_stop ) { if( ONIKA_CU_PROF_START() != onikaSuccess ) { ldbg<<"Warning, profiler start returned an error"<<std::endl; } }
...
#         pragma omp taskgroup
  ...
  void OperatorNode::run_epilog()
  {
    ...
    { 
      ...
      if( m_gpu_profile_start_stop ) { if( ONIKA_CU_PROF_STOP() != onikaSuccess ) { ldbg<<"Warning, profiler stop returned an error"<<std::endl; } }
    }
