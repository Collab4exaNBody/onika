...
function(onika_add_plugin PluginName dirname)
  ...
  if(${ARGC} GREATER_EQUAL 3)
    set(PluginSuffix ${ARGV2})
  else()
    set(PluginSuffix "-plugin")
  endif()
  ...
  if(ONIKA_BUILD_${PluginName}_FORCED)
    set(ONIKA_BUILD_${PluginName} ${ONIKA_BUILD_${PluginName}_DEFAULT})
  else()
  ...
  if(ONIKA_BUILD_${PluginName})
    ...
    foreach(EXT_SRC ${${PluginName}_EXTERNAL_SRCS})
      if(EXT_SRC MATCHES ".*\.cu")
        ...
        list(APPEND ${PluginName}_SCRS_CU ${EXT_SRC})
      endif()
    endforeach()
    list(APPEND ${PluginName}_SCRS ${${PluginName}_EXTERNAL_SRCS})

    if(ONIKA_COMPILE_CXX_AS_CU)
      if(ONIKA_BUILD_CUDA)
        if(ONIKA_USE_HIP)
          set_source_files_properties(${${PluginName}_SCRS_CXX} PROPERTIES LANGUAGE HIP)
          set_source_files_properties(${${PluginName}_SCRS_LIB_CXX} PROPERTIES LANGUAGE HIP)
          ...
          set_source_files_properties(${${PluginName}_SCRS_CXX} PROPERTIES LANGUAGE CUDA)
          set_source_files_properties(${${PluginName}_SCRS_LIB_CXX} PROPERTIES LANGUAGE CUDA)
          set_source_files_properties(${${PluginName}_SCRS_CU} PROPERTIES LANGUAGE CUDA)
          set_source_files_properties(${${PluginName}_SCRS_LIB_CU} PROPERTIES LANGUAGE CUDA)        
        endif()
      else()
        set_source_files_properties(${${PluginName}_SCRS_CXX} PROPERTIES LANGUAGE CXX)
        set_source_files_properties(${${PluginName}_SCRS_LIB_CXX} PROPERTIES LANGUAGE CXX)
    ...
    if(${PluginName}_SCRS_LIB)
      ...
      set_target_properties(${${PluginName}_SHARED_LIB} PROPERTIES LIBRARY_OUTPUT_DIRECTORY ${PROJECT_BINARY_DIR}/lib)
      install(TARGETS ${${PluginName}_SHARED_LIB} DESTINATION lib)
    elseif(EXISTS ${dirname}/include)
      set(${PluginName}_SHARED_LIB ${PluginName})
      set(${PluginName}_SHARED_LIB ${${PluginName}_SHARED_LIB} PARENT_SCOPE)
      #message(STATUS "component create interface lib target ${${PluginName}_SHARED_LIB}")
    ...
    if(${PluginName}_SCRS)
      ...
      #message(STATUS "create plugin lib target ${${PluginName}_PLUGIN_LIB}")
      ...
      target_link_options(${${PluginName}_PLUGIN_LIB} PRIVATE ${${PluginName}_LINK_OPTIONS})
      set_target_properties(${${PluginName}_PLUGIN_LIB} PROPERTIES LIBRARY_OUTPUT_DIRECTORY ${PROJECT_BINARY_DIR}/plugins)
