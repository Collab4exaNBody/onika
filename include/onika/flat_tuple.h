...
/*** flat_tuple.hxx generation script ***
python3 > flat_tuple.hxx <<- EOF
NMAX=40
...
print("static inline constexpr size_t FlatTupleMaxElements = %d;\n" % NMAX)
print("template<class... T> struct FlatTuple { static_assert( sizeof...(T) <= FlatTupleMaxElements ); };")
